...
- **Language & Framework:** Deduza a partir de arquivos de manifesto (como `requirements.txt`, `package.json`, `Package.swift`, `pyproject.toml`, `build.gradle`, `go.mod`, `Podfile.lock`, etc.) e das declarações de importação. Não adivinhe baseando-se apenas nas extensões dos arquivos.
...
- **Current Architecture:** Identifique a organização estrutural. A lógica está agrupada em scripts soltos/monolíticos ou já possui separação clara (Models, Controllers, Views, Services, etc.)? Aponte a topologia atual.
...
<Outputs>
Imprima EXATAMENTE como abaixo, preenchendo os valores entre `< >` sem alterar o layout.
IMPORTANTE: Retorne APENAS o template. Não inclua saudações, explicações ou texto introdutório.
